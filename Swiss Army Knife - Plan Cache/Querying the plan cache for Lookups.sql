...
--  If a lookup is being done for a small subset of columns of a table, it may be a chance to review the existing non-clustered indexes, 
--  namely the one that is being used in conjunction with the lookup, and possibly add included columns to avoid these lookups.
